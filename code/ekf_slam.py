...
def init_landmarks(init_measure, init_measure_cov, init_pose, init_pose_cov):
    ...
    for i in range(k):
        ...
        # computing lx, ly >>>>>>>>>>>>>>>>>>>>>>>>>
        totalAngle = warp2pi(init_measure[2*i] + theta)
        landmark[2*i]   = init_pose[0] + (init_measure[2*i+1]) * np.cos(totalAngle)
        landmark[2*i+1] = init_pose[1] + (init_measure[2*i+1]) * np.sin(totalAngle)
        # <<<<<<<<<<<<<<<<<<<< computing lx, ly 

        # computing landmark cov >>>>>>>>>>>>>>>>>>>>>>>>>
        beta = init_measure[2*i,0]
        r = init_measure[2*i+1,0]
        
        # jacob of landmark_x w.r.t beta
        dldb1 = -r*np.sin(warp2pi(beta+theta))
        # jacob of landmark_x w.r.t r
        dldr1 = np.cos(warp2pi(beta+theta))
        # jacob of landmark_y w.r.t beta
        dldb2 = r*np.cos(warp2pi(beta+theta))
        # jacob of landmark_y w.r.t r
        dldr2 = np.sin(warp2pi(beta+theta))
        
        landmark_jacob = np.array([ [dldb1,dldr1] , [dldb2,dldr2] ])
        
        landmark_cov[2*i:2*i+2,2*i:2*i+2] = landmark_jacob @ init_measure_cov @ landmark_jacob.T
        # <<<<<<<<<<<<<<<<<<<<<< computing landmark cov 
...
def predict(X, P, control, control_cov, k):
    ...
    # translate command in robot's x+ axis
    dt = control[0]
    # rotate command 
    alpha = control[1]
    # wrap the angle
    ...
    Move = np.hstack([np.eye(3),np.zeros((3,2*k))])
    motion = np.array([dt*np.cos(theta),dt*np.sin(theta),alpha])
    F = np.hstack([np.eye(3),np.zeros((3,2*k))])
    bottom = np.hstack([np.zeros((2*k,3)),np.eye(2*k)])
    F = np.vstack([F,bottom])
    # F top left 3 x 3 is jacobian of g() w.r.t pose x
    F[0,2] = -dt*np.sin(theta)
    F[1,2] =  dt*np.cos(theta)
    # print("F = ",F)
    
    # L: jacobian of g w.r.t control noise - (ex,ey,ea)
    L = np.array([[np.cos(theta),-np.sin(theta) ,0],
                  [np.sin(theta), np.cos(theta) ,0],
                  [0            ,             0 ,1]])

    X_pre = X + Move.T @ motion
    # print("X_pre=",X_pre)

    # Rt = LQL.T
    LQL_T1 = L @ control_cov @ L.T
    LQL_T2 = np.hstack([LQL_T1,np.zeros((3,2*k))])
    LQL_T = np.vstack([LQL_T2,np.zeros((2*k,2*k+3))])
    
    # State_cov_pre_t = Gt state_cov_t Gt.T + Rt
    P_pre = F @ P @ F.T + LQL_T
...
def update(X_pre, P_pre, measure, measure_cov, k):
    ...
    for i in range(k):
        ...
        # dx = lx - x, dy = ly - y
        Hp = np.asarray([[ dy/r2 , -dx/r2,-1],                      # 2 X 3
                         [ -dx/r , -dy/r , 0]])   
        # print("Hp size= ", np.shape(Hp))
        Hl = np.asarray([[-dy/r2, dx/r2],                           # 2 X 2
                         [dx/r , dy/r]])
        # print("Hl size = ", np.shape(Hl))
        
        
        H = np.hstack([Hp,Hl]) @ scaleH                              # (2 X 5)  * (5 X 15)            = 2 X 15
        # print( " --------- ")
        # print(" H = \n ",H)
        # print( " --------- ")
        S = H @ P_pre @ H.T + measure_cov                           # (2 X 15) * (15 X 15) * (15X 2) = 2 X 2

        K = P_pre @ H.T @ np.linalg.inv(S)                          # (15 X 15) * (15 X 2) * (2 X 2) = 15 X 2
        # print(" K SHAPE = ", np.shape(K))

        X_pre = X_pre + K@(measure[2*i:2*i+2] - measure_pre)        # (15 X 2) * (2 X 1)             = 15 X 1
        P_pre = (np.eye(3+2*k) - K @ H) @ P_pre                     # (15 X 2) * (2 X 15)            = 15 X 15      
...
def evaluate(X, P, k):
    ...
    for i in range(k):
        # euclidian
        dx = l_true[2*i]   - landMark_est[2*i]
        dy = l_true[2*i+1] - landMark_est[2*i+1]
        eucl_d = np.sqrt(dx*dx+dy*dy)
        euclidian.append(eucl_d)
        # mahalanobis
        dist = np.vstack([dx,dy])
        mahal_d = np.sqrt(dist.T @ np.linalg.inv(P[3+ 2*i: 3+ 2*i + 2,3 + 2*i: 3+ 2*i + 2]) @ dist)
        mahalanobis.append(mahal_d[0])

    euclidian = np.asarray(euclidian)
